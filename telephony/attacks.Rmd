...
## Installer ou télécharger les Rainbow Tables
...
## Utiliser Kraken (manuel et avec behemoth.py)
...
### Télécharger les tables Rainbow
...
# Télécharger les tables (~1,7To)

# Installer les tables (TableConvert ou install.py)

# Éditer delta_config.h (chemins absolus/UUID)

# Générer kernel : ./genkernel32.sh > slice.c

# Lancer : paplon.py, oclvankus.py, delta_client.py

# Connexion : telnet localhost 1578, commande "crack"

# Tune (stats, kernel, stockage)
```
...
# LTE Redirection Attack — Principe et démonstration expérimentale

Les réseaux cellulaires modernes (LTE/4G et 5G) ont été conçus pour améliorer la sécurité des communications mobiles.  
Cependant, afin de maintenir la compatibilité avec les générations précédentes, les téléphones restent capables de basculer vers des technologies plus anciennes comme **UMTS (3G)** ou **GSM/EDGE (2G)**.

Cette compatibilité introduit une surface d’attaque connue sous le nom de :
